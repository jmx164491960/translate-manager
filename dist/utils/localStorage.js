...
export default {
    ...
    maxAge: function (age) {
        // 将传入的天数转为时间戳
        var time = age * 24 * 60 * 60 * 1000;
        this.age = time;
        return this;
    },
    // 存储 顺便存时间戳
    set: function (name, json, hasExpire) {
    ...
    // 添加
    add: function (name, data, length) {
        if (typeof data === 'string' || typeof data === 'number') {
            if (typeof data === 'number') {
                data += '';
            }
            var oldValue = this.get(name);
            var oldValueArr = oldValue ? oldValue.split('|') : [];
            var oldValueArrL = oldValueArr.length;
            if (oldValueArrL === 0) {
                this.set(name, data);
            }
            else {
    ...
    isExpired: function (name) {
        var init = localStorage.getItem(name), _time = 0, iTime = new Date().getTime(), timeLength = 0;
        if (init) {
    ...
    // 判断是否进行初始化过，且提供简单回调
    isInit: function (name, fn) {
        var data = '', age = this.age;
        // 如果没过期直接取，过期删除
    ...
    // 未过期添加新内容，过期则覆盖之前的内容
    update: function (name, datakey, newContent) {
